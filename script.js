document.addEventListener('DOMContentLoaded', function () {
  ...
  if (animatableElements.length > 0) {
    ...
    defaultStyle.textContent = [
      '.animate, .fade-in, .fade-in-up, .scroll-animate, [data-animate] {',
      '  opacity: 0;',
      '  transform: translateY(24px);',
      '  transition: opacity 0.6s ease, transform 0.6s ease;',
      '}',
      '.animate.is-visible, .fade-in.is-visible, .fade-in-up.is-visible,',
    ...
    if ('IntersectionObserver' in window) {
      ...
      animatableElements.forEach(function (el, index) {
        if (!el.hasAttribute('data-delay') && index > 0) {
          const siblings = Array.from(el.parentElement.children).filter(function (child) {
            return child.matches(ANIMATION_SELECTORS.join(', '));
          });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "לופדיגיטל משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '85f78627-12a0-4eee-974c-e0c293b527e1',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '85f78627-12a0-4eee-974c-e0c293b527e1';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '85f78627-12a0-4eee-974c-e0c293b527e1';
